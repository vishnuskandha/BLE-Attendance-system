...
".s{background:#f9f9f9;padding:12px;margin:8px 0;border-radius:5px;border-left:3px solid #667eea}"
"select,input{width:100%;padding:10px;margin:5px 0;border:2px solid #ddd;border-radius:5px;box-sizing:border-box}"
"button{background:#667eea;color:#fff;padding:12px;border:none;border-radius:5px;cursor:pointer;width:100%;margin-top:10px}"
"button:hover{opacity:0.9}.w{background:#fef3c7;padding:12px;margin:10px 0;border-radius:5px;border-left:3px solid #f59e0b}"
".on{color:#10b981;font-weight:bold}.off{color:#6b7280}a{color:#667eea;text-decoration:none}"
...
void setupRTC() {
  ...
  if (rtcTimeSet) prefs.putBool("set", true);
  prefs.end();
}
...
String getDateStr() {
  if (!rtcReady || !rtcTimeSet) return "NOT_SET";
  DateTime now = rtc.now();
  char buf[16];
...
int getCurrentPeriod() {
  if (!rtcReady || !rtcTimeSet) return 0;
  DateTime now = rtc.now();
  ...
  for (int i = 0; i < PERIOD_COUNT; i++) {
    ...
    if (currentMin >= startMin && currentMin < endMin) return i + 1;
  }
  return 0;
}

bool isOnDutyActive(StudentBeacon& student) {
  if (!student.onDuty || !rtcReady || !rtcTimeSet) return false;
...
void sendAttendance(const StudentBeacon& s, bool present, bool onDuty) {
  if (!wifiReady || WiFi.status() != WL_CONNECTED || !rtcTimeSet) return;
...
void handleRoot() {
  ...
  html += "<p style='text-align:center;font-size:14px'>Time: " + getTimeStr() + " | " + getDateStr() + "</p>";
  html += "<form method='POST' action='/grant'><div class='s'><h3>Student:</h3><select name='id' required>";
  html += "<option value=''>-- Select --</option>";
  
  for (int i = 0; i < REG_COUNT; i++) {
    html += "<option value='" + String(registry[i].id) + "'>" + registry[i].name + "</option>";
  }
  ...
  for (int i = 0; i < REG_COUNT; i++) {
    ...
    html += registry[i].onDuty ? "<span class='on'>ON DUTY</span>" : "<span class='off'>Normal</span>";
    html += "</div>";
  }
...
void handleSetTimePage() {
  ...
  html += "<div class='w'>Current: " + getDateStr() + " " + getTimeStr() + "</div>";
  html += "<form method='POST' action='/savetime'>";
  html += "<div class='s'><label>Date:</label><input type='date' name='date' value='" + currentDate + "' required></div>";
  html += "<div class='s'><label>Time (24h):</label><input type='time' name='time' value='" + currentTime + "' required></div>";
  html += "<button type='submit'>Save to RTC</button>";
  html += "<a href='/'><button type='button' style='background:#6b7280;margin-top:5px'>Back</button></a></form>";
  html += "<p style='font-size:12px;color:#666;margin-top:15px'>Note: RTC has backup battery for persistent time.</p>";
  html += "</div></body></html>";
  server.send(200, "text/html", html);
}
...
void handleSaveTime() {
  if (!rtcReady) {
    server.send(500, "text/html", "<h1>Error: RTC not available</h1><a href='/'>Back</a>");
    return;
  }
  ...
  if (server.hasArg("date") && server.hasArg("time")) {
    ...
    html += "<div class='s' style='text-align:center'><h2>" + getDateStr() + "</h2><h2>" + getTimeStr() + "</h2></div>";
    html += "<p style='text-align:center'>RTC will maintain time even when powered off.</p>";
    html += "<a href='/'><button>Back to Portal</button></a>";
    html += "</div></body></html>";
    server.send(200, "text/html", html);
  } else {
    server.send(400, "text/html", "<h1>Error</h1><a href='/settime'>Try Again</a>");
  }
}

void handleGrant() {
  if (!rtcTimeSet) {
    server.send(400, "text/html", "<h1>Error: Set RTC time first</h1><a href='/settime'>Set Time</a>");
    return;
  }
  ...
  if (server.hasArg("id") && server.hasArg("type")) {
    ...
    for (int i = 0; i < REG_COUNT; i++) {
      if (registry[i].id == studentId) {
        ...
        html += "<div class='s' style='text-align:center'><h2>" + registry[i].name + "</h2>";
        html += "<p>On-duty for <strong>" + String(duration) + " minutes</strong></p></div>";
